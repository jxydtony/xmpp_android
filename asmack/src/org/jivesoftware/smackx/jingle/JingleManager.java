...
public class JingleManager implements JingleSessionListener {
    ...
    */
    ...
    private void initJingleSessionRequestListeners() {
                ...
                if (pin instanceof IQ) {
                    ...
                    if (iq.getType().equals(IQ.Type.SET)) {
                        if (iq instanceof Jingle) {
                            ...
                            if (jin.getAction().equals(JingleActionEnum.SESSION_INITIATE)||jin.getAction().equals(JingleActionEnum.SESSION_TERMINATE)) {
                                return true;
                            }
                        }
                    }
        ...
        connection.addPacketListener(new PacketListener() {
            public void processPacket(Packet packet) {
            	...
            	if(jin.getAction().equals(JingleActionEnum.SESSION_TERMINATE))
	            	{
	            		//get the specified session of the specified sid
            		   for(JingleSession jingleSession:jingleSessions)
            		   {
            			   if(jingleSession.getSessionState().equals(JingleSessionStatePending.getInstance())
            					   &&jingleSession.getSid().equals(jin.getSid()))
            			   {
	   								jingleSession.triggerSessionClosed("other closed Session!");
    ...
    public JingleSession createIncomingJingleSession(JingleSessionRequest request) throws XMPPException {
        ...
        JingleSession session = new JingleSession(connection,request.getFrom(), connection.getUser(), request.getSessionID(),jingleMediaManagers);
