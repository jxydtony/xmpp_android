...
//            	Log.d("done", ""+done);
//            	Log.d("done","after thread sleep:"+new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()));
//            	Log.d("done","lastActive:"+new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date(lastActive)));
				synchronized (writer) {
					// Send heartbeat if no packet has been sent to the server
					// for a given time
					if (System.currentTimeMillis() - connection.packetReader.lastActive >= delay) {
						try {
							// Log.d("done", "start send ping:"+new
							// SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()));
							Packet ping = new PingPacket();
							// Wait up to a certain number of seconds for a
							// reply from the server.
							final PacketCollector collector = connection
									.createPacketCollector(new PacketIDFilter(
											ping.getPacketID()));
							connection.sendPacket(ping);
							new Thread(){
								public void run() {
									IQ response = (IQ) collector
											.nextResult(SmackConfiguration
													.getPacketReplyTimeout());
									collector.cancel();
									if (response == null&&connection.isConnected()) {
										done = true;
										connection.packetReader
												.notifyConnectionError(new XMPPException(
														"no response from server for ping"));
									}
								}
							}.start();
